...
SCRIPT = Path(__file__).resolve().parents[1] / "omarchy.py"
...
class RestoreTest(unittest.TestCase):
    def setUp(self):
        self.temporary = tempfile.TemporaryDirectory(prefix="dotfiles test's ")
        self.addCleanup(self.temporary.cleanup)
        self.home = Path(self.temporary.name)
        self.args = argparse.Namespace(home=self.home, apply=False, overwrite_local=False, with_hardware=False)
    ...
    def backups(self):
        return set((self.home / ".dotfiles-backup").glob("restore-*"))
    ...
    def test_printed_undo_quotes_shell_paths(self):
        ...
        self.assertEqual(omarchy.shlex.split(command)[1], str(self.home / "my checkout/script/omarchy.py"))

    def test_main_config_is_preserved_and_hardware_is_opt_in(self):
